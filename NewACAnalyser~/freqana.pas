...
type
  ...
  TBeatSyncInfo       = record
    SMaxBeatValue         : MVFloat;
    SMaxBeatPos,SUsedSince: MVInt;
  end;
  ...
  {strict} private
    ...
    procedure SetBeatIdleTime(const Value: MVInt);
    function GetBeatIdleTime: MVInt;
  public
    FWData        : TSD2WData;
...
var
  ...
  begin
    if NewSoundEnergy>FNormBeatDiff{.Peak}*FBeatSensibility then begin
      ...
      if FTickCount-FBeat.BeatStart{.Peak}>3 then FBeat.Free:=true {Include(FBeat.Free,btPeak)};
    end;
  end;
...
procedure TSpectrumData2.DoGetBeatPos; inline;
begin
  with FBeat do begin
    BeatTime+=1.0;
    if IsZero(BeatLength) then exit;
    if BeatTime>=BeatLength then begin
      Pos:=(Pos+1) mod Max;
      BeatTime:=Frac(BeatTime/BeatLength)*BeatLength;
    end;
  end;
end;

procedure TSpectrumData2.DoSync;
var
  ABeatVal: MVFloat;
begin
  ABeatVal:=1.0;
...
procedure TSpectrumData2.SyncTap;
begin
  with FBeat do begin
    if BeatTime<BeatLength/2
      then BeatTime:=0.0
      else BeatTime:=BeatLength;
  end;
end;

procedure TSpectrumData2.DoGetBPM; {inline;}
var
  I,J,Subband,AOverCount,AHistoryIndex,AActiveIndex,AActiveIndexStart,ABPMPos,ABPMCount,ALastBPM,ANoBeatCount: Integer;
  AHistory,AActive                                                                              : TSD2Bits;
  ABufPointer                                                                                   : Pointer;
  ABuf                                                                                          : PSD2BufElem absolute ABufPointer;
  AAverageBeatEquality,ABPMFac,ATempMaxEquality                                                 : MVFloat;
...
begin
  ...
  for I:=0 to FFSBeatBuffers.SBeatEqualityCount-1 do begin
    ...
    if ABuf^>FFSBeatBuffers.SAverageBeatEquality*FBPMSensibility then begin
      //if I-ABPMPos>2 then begin
      if ATempMaxEquality<ABuf^ then begin
        ATempMaxEquality:=ABuf^;
        ABPMPos:=I;
      end;
        //Inc(ABPMCount);
        //FBPMThread.FBPMList.AddBPM(ABPMFac/(I-ALastBPM{BPMPos}),I-ALastBPM{BPMPos}); {ALastBPM ???}
        //APreBPM+=I-ALastBPM;
      //end;
      //ABPMPos:=I;
    end else begin
      Inc(ANoBeatCount);
      if (ABPMPos>=0) and (ANoBeatCount>2) then begin
        if ANotFirst then begin
          Inc(ABPMCount);
          FBPMThread.FBPMList.AddBPM(ABPMFac/(ABPMPos-ALastBPM{BPMPos}),ABPMPos-ALastBPM{BPMPos}); {ALastBPM ???}
        end else ANotFirst:=true;
        ALastBPM:=ABPMPos;
        ABPMPos:=-1;
        ANoBeatCount:=0;
        ATempMaxEquality:=0.0;
  ...
  if ABPMCount>0 then begin
    ...
    if FBeatOptimizer then begin
      FBPMThread.FBPMList.FindBPM(FBPMThread.FramesNeeded);
      FBPM:=FBPMThread.FBPMList.BPM;
      //FBPM:=120;
    end else FBPM:=ABPMFac/(ALastBPM{BPMPos}/ABPMCount);
  ...
  if (FBPM<10.0) or (FBPM>1500.0) then FBPM:=0.0;
  ...
  FBPMThread.FBPMHistoryList.DeleteBPM(AHistoryBuf^,1);
  AHistoryBuf^:=FBPM;
  FBPMThread.FBPMHistoryList.AddBPM(AHistoryBuf^,1);
  ...
  FBPM*=FBPMFac;
  FBeat.BeatLength:=1/(FBPM/ABPMFac);
end;
...
begin
  ...
  with FFSBeatBuffers do begin
    if FBeatSubbandCount>0 then begin
      FreeMem(SAverageSoundEnergys,SSubbandSize{FBeatSubbandCount*SD2BufElemSize});
  ...
  with FFSBeatBuffers do begin
    ...
    GetMem(SAverageSoundEnergys,SSubbandSize{ABeatSubbandCount*SD2BufElemSize});
    GetMem(SBandWidths,ABeatSubbandCount*SD2IntBufElemSize);
    GetMem(SBeatEquality,SBeatEqualityCount*SD2BufElemSize);
  ...
  FBPMThread.FBPMList.Size:=Trunc(sqrt(0.25+(2*{FFSBeatBuffers.SBeatEqualityCount}ABeatHistoryCount))-0.5)+10; //berechnet, wie viele Zahlen summiert werden müssen, um auf ABeatHistoryCount zu kommen (so viele verschiedene Beats gibt es maximal)
  FBPMThread.FBPMHistoryList.Size:=FBPMHistoryCount+10{FBPMList.Size};
...
procedure TSpectrumData2.SetBeatIdleTime(const Value: MVInt);
begin
  FBPMThread.FSleepTime:=Value;
end;

function TSpectrumData2.GetBeatIdleTime: MVInt;
begin
...
function TSpectrumData2.GetBeatAtH(const Subband,History: MVInt): Boolean; stdcall;
begin
...
function TSpectrumData2.GetBeatAt(const Subband: MVInt): Boolean; stdcall;
begin
  with FFSBeatBuffers do Result:=SD2Bits_Get(SIsBeatAt,Subband);
end;

function TSpectrumData2.GetBeatState: MVBeatState; stdcall;
begin
  Result:=FBeat;
end;

function TSpectrumData2.GetBPM: MVFloat; stdcall;
begin
  Result:=FBPM;
end;

function TSpectrumData2.GetBPMFound: TBPMFoundMode; stdcall;
begin
  Result:=FBPMFound;
end;
