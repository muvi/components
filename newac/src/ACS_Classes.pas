...
(* $Id: ACS_Classes.pas 1174 2010-02-04 20:11:57Z andrei.borovsky $ *)
...
{$ENDIF}
  ...
  ACS_Types, ACS_Procs, ACS_Tags, Windows; (***)
...
(* Class: TAuInput
  ...
  protected
    ...
    DataCS : SyncObjs.TCriticalSection;
...
(* Class: TAuOutput
  ...
  protected
    ...
    RunCS : SyncObjs.TCriticalSection;
...
(* Class: TAuStreamedOutput
  ...
  protected
    OpenCS : SyncObjs.TCriticalSection;
...
var
  ...
  LoadLibCS : SyncObjs.TCriticalSection;
...
begin
  ...
  begin
    ...
    DataCS := SyncObjs.TCriticalSection.Create;
  end;
...
constructor TAuOutput.Create;
  begin
    ...
    begin
      FStopped := False;
      RunCS := SyncObjs.TCriticalSection.Create;
...
//      if (not FStreamAssigned) then FStream := nil;
  ...
  begin
    inherited Create(AOwner);
    OpenCS := SyncObjs.TCriticalSection.Create;
  end;
...
//       FastCopyMem(@P[Result], P1, r);
  ...
  begin
    {$IFDEF UNICODE}
    if (*F*)Handle <> INVALID_HANDLE_VALUE then FileClose((*F*)Handle);
    {$ENDIF}
    {$IFNDEF UNICODE}
    if (*F*)Handle >= 0 then FileClose((*F*)Handle);
    {$ENDIF}
    inherited Destroy;
  end;
