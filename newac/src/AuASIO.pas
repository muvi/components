...
(* $Id: AuASIO.pas 1169 2010-02-03 13:50:15Z andrei.borovsky $ *)
...
uses
  SysUtils, Classes, Forms, SyncObjs, {FastMove,} ACS_Types, ACS_Procs, ACS_Classes, LCLIntf, AsioList, OpenASIO, ASIO, Math, Windows; (***)
...
//<<<<<<< .mine//    ACS : TCriticalSection;
    ACS : SyncObjs.TCriticalSection;
    _Prefetched : Boolean;
//>>>>>>> .theirs//    FOnUnderrun : TUnderrunEvent;
    FLatency: LongInt {LongWord};
    FBufferSize : LongWord;
    FSupportedChannels : LongInt{LongWord}; (***)
...
begin
  ACS := SyncObjs.TCriticalSection.Create;
  FInput.Init;
...
var
  i                      : LongWord;
  Dummie, min, max, pref : LongInt; {LongWord} (***)
  chi                    : TAsioChannelInfo;
begin
  if ASIOStarted then Exit;
  if (FDeviceNumber >= FDeviceCount) then raise EAuException.Create('Invalid ASIO device number');
