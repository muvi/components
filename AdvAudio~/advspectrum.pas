...
uses
  ...
  Forms, SimpleSpectrumData, SpectrumData, FreqAna
  {$IFDEF MSWINDOWS}
  ,Windows
  {$ENDIF};
...
//  if (Buffer <> nil) and (Bytes >= FBytesToRead) {or (FElapsed<FInterval)} then {FGetDataProc:=GetData_FFT else begin}
    //FElapsed:=0.0;
    FFFTThread.PushBuf(Buffer,Bytes);
    if FSpectrumData<>nil then begin
      case FSampleSize of
        1 : ByteToSingle(PBuffer8(Buffer), FSpectrumData.FWData, FFFTThread.FSampleBufSize);
        2 : SmallIntToSingle(PBuffer16(Buffer), FSpectrumData.FWData, FFFTThread.FSampleBufSize);
        3 : Int24ToSingle(PBuffer8(Buffer), FSpectrumData.FWData, FFFTThread.FSampleBufSize);
        4 : Int32ToSingle(PBuffer32(Buffer), FSpectrumData.FWData, FFFTThread.FSampleBufSize);
      end;
    end;
  //end;
...
procedure  TAdvancedSpectrumIndicator.AssignSpectrumData(ASpectrumData: TSpectrumData2);
begin
